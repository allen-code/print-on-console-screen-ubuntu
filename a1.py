...
def kill():
    ...
    io_system.remove_window_from_process(process)


    if process.state == State.waiting:
        index = the_dispatcher.processList.index(process)
        the_dispatcher.processList[index] = False

    if process.state == State.runnable:
        
        the_dispatcher.lock.acquire         
        index = the_dispatcher.processStack.index(process)
        the_dispatcher.processStack.pop(index)

        for i in range(index, len(the_dispatcher.processStack)):
            process_to_move = the_dispatcher.processStack[i]
            io_system.move_process(process_to_move, i)
        the_dispatcher.dispatch_next_process()
        the_dispatcher.lock.release
