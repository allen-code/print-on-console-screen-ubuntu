...
class IO_Sys():
    ...
    def read(self, process):
        ...
        self.the_dispatcher.lock.acquire


        if self.the_dispatcher.processStack:
            self.the_dispatcher.processStack[-1].tos = False

        process.state = State.runnable


        index = self.the_dispatcher.processList.index(process)
        self.the_dispatcher.processList[index] = False

        self.the_dispatcher.processStack.append(process)

        if self.the_dispatcher.processStack:
            self.the_dispatcher.processStack[-1].tos = True

        self.move_process(process, len(self.the_dispatcher.processStack) - 1 )        

        self.the_dispatcher.lock.release
